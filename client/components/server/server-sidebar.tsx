...
export const ServerSidebar = async ({ serverId }: ServerSidebarProps) => {
  const response = await serversApi.getServerAndProfileById({ id: serverId });

  const { server, profile } = response.data.data;
      ...
      <ScrollArea className="flex-1 px-3">
            ...
            data={[
              {
                ...
                data: textChannels?.map(
                  (channel: { id: any; name: any; type: string | number }) => ({
                    id: channel.id,
                    name: channel.name,
        ...
        {!!textChannels?.length && (
          <div className="mb-2">
            <ServerSection
              ...
              channelType={ChannelType.TEXT as ChannelType}
              role={role}
              label="Text Channels"
            />
        ...
        {!!audioChannels?.length && (
          <div className="mb-2">
            <ServerSection
              sectionType="channels"
              channelType={ChannelType.AUDIO as ChannelType}
              role={role}
              label="Voice Channels"
            />
            <div className="space-y-[2px]">
        ...
        {!!videoChannels?.length && (
          <div className="mb-2">
            <ServerSection
              sectionType="channels"
              channelType={ChannelType.VIDEO as ChannelType}
              role={role}
              label="Video Channels"
            />
            <div className="space-y-[2px]">
